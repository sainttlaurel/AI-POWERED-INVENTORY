...
class Forecasting {
    ...
    public function calculateForecast($product_id) {
        ...
        $query = "SELECT SUM(quantity) as total_sales, COUNT(DISTINCT DATE(sale_date)) as days_with_sales 
                  FROM sales 
                  WHERE product_id = :pid AND sale_date >= DATE_SUB(NOW(), INTERVAL 30 DAY)";
        ...
        // Reorder suggestion (monthly forecast + buffer)
        $reorder_suggestion = ceil($forecast_monthly * 1.2);
        
        // Update or insert forecast data
        $query = "INSERT INTO forecast_data (product_id, avg_daily_sales, forecast_weekly, forecast_monthly, predicted_depletion_days, reorder_suggestion) 
                  VALUES (:pid, :avg, :weekly, :monthly, :depletion, :reorder)
                  ON DUPLICATE KEY UPDATE 
                  avg_daily_sales = :avg, forecast_weekly = :weekly, forecast_monthly = :monthly, 
                  predicted_depletion_days = :depletion, reorder_suggestion = :reorder";
