...
// Recent transactions
$recent_sales = $db->query("SELECT s.*, p.product_name FROM sales s JOIN products p ON s.product_id = p.id ORDER BY s.sale_date DESC LIMIT 5")->fetchAll(PDO::FETCH_ASSOC);

// Low stock products
$low_stock_products = $db->query("SELECT * FROM products WHERE stock_quantity <= reorder_level ORDER BY stock_quantity ASC LIMIT 5")->fetchAll(PDO::FETCH_ASSOC);

// Sales data for chart (last 7 days)
$sales_data = $db->query("SELECT DATE(sale_date) as date, SUM(total_price) as total FROM sales WHERE sale_date >= DATE_SUB(CURDATE(), INTERVAL 7 DAY) GROUP BY DATE(sale_date) ORDER BY date")->fetchAll(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html lang="en">
<head>
                    ...
                    <div class="col-md-3">
                        <div class="card text-white bg-danger">
                            <div class="card-body">
                                ...
                                <h2><?php echo $stats['out_of_stock']; ?></h2>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3">
