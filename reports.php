...
try {
    switch ($report_type) {
        case 'sales':
            ...
            // Get all sales and process them properly
            $sales_query = "SELECT s.*, p.product_name, p.barcode, sup.name as supplier_name 
                           FROM sales s 
            ...
            foreach ($sales_data as $sale) {
                ...
                $sale_items_query = "SELECT si.*, p.product_name, sup.name as supplier_name 
                                    FROM sale_items si 
                ...
                $sale_amount = $sale['total_price'] ?? 0;
                $total_sales += $sale_amount;
                
                if (!empty($sale_items)) {
                    // Multi-item sale
                    if (count($sale_items) > 1) {
                        $total_qty = array_sum(array_column($sale_items, 'quantity'));
                        $total_quantity += $total_qty;
                        
                        $report_data[] = [
                            'id' => $sale['id'],
                            'created_at' => $sale['created_at'],
                            'product_name' => 'Multi-item Sale (' . count($sale_items) . ' items)',
                            'barcode' => 'SALE-' . str_pad($sale['id'], 6, '0', STR_PAD_LEFT),
                            'quantity' => $total_qty,
                        ...
                        $report_data[] = [
                            ...
                            'customer_name' => $sale['customer_name'],
                            'payment_method' => $sale['payment_method'],
                            'sale_type' => 'single_from_items'
                        ];
                    }
                } else if ($sale['product_id']) {
                    // Single item sale (old format)
                    $total_quantity += $sale['quantity'] ?? 0;
                    
                    $report_data[] = [
                        'id' => $sale['id'],
                        'created_at' => $sale['created_at'],
                        'product_name' => ($sale['product_name'] ?? 'Unknown Product') . ' (' . ($sale['supplier_name'] ?? 'No Brand') . ')',
                        'barcode' => $sale['barcode'] ?? 'N/A',
                        'quantity' => $sale['quantity'] ?? 0,
                        'total_price' => $sale_amount,
                        'customer_name' => $sale['customer_name'],
                        'payment_method' => $sale['payment_method'],
                        'sale_type' => 'single_old'
                    ];
                } else {
                    // Sale without product info
                    $report_data[] = [
                        'id' => $sale['id'],
                        'created_at' => $sale['created_at'],
                        'product_name' => 'Sale #' . $sale['id'],
                        'barcode' => 'SALE-' . str_pad($sale['id'], 6, '0', STR_PAD_LEFT),
                        'quantity' => 0,
                        'total_price' => $sale_amount,
                        'customer_name' => $sale['customer_name'],
        ...
        case 'profit_loss':
            ...
            // Cost of Goods Sold (estimated at 70% of selling price)
            // Handle both single sales and sale_items
            $cogs_query = "
                SELECT SUM(cost_estimate) as total_cogs FROM (
                    SELECT SUM(s.quantity * p.price * 0.7) as cost_estimate
                    FROM sales s 
                    ...
                    SELECT SUM(si.quantity * p.price * 0.7) as cost_estimate
                    FROM sale_items si
            ...
            $category_query = "
                SELECT category, SUM(revenue) as revenue, SUM(cogs) as cogs FROM (
                    SELECT c.name as category, SUM(s.total_price) as revenue, SUM(s.quantity * p.price * 0.7) as cogs
                    FROM sales s 
                    JOIN products p ON s.product_id = p.id 
                    ...
                    SELECT c.name as category, SUM(s.total_price) as revenue, SUM(si.quantity * p.price * 0.7) as cogs
                    FROM sale_items si
                    JOIN sales s ON si.sale_id = s.id
                    JOIN products p ON si.product_id = p.id
                    JOIN categories c ON p.category_id = c.id
        ...
        case 'supplier_performance':
            ...
            $query = "
                ...
                LEFT JOIN (
                    SELECT p.supplier_id, SUM(COALESCE(s.total_price, 0)) as revenue, COUNT(s.id) as transactions
                    FROM sales s
                    JOIN products p ON s.product_id = p.id
                    WHERE s.product_id IS NOT NULL AND DATE(s.created_at) BETWEEN ? AND ?
                    GROUP BY p.supplier_id
                    
                    UNION ALL
                    
                    SELECT p.supplier_id, SUM(s.total_price) as revenue, COUNT(DISTINCT s.id) as transactions
                    FROM sale_items si
                    JOIN sales s ON si.sale_id = s.id
                    JOIN products p ON si.product_id = p.id
                    WHERE DATE(s.created_at) BETWEEN ? AND ?
        ...
        case 'abc_analysis':
            ...
            $query = "
                ...
                LEFT JOIN categories c ON p.category_id = c.id 
                LEFT JOIN (
                    SELECT product_id, SUM(COALESCE(total_price, 0)) as revenue, SUM(quantity) as units_sold
                    FROM sales 
                    WHERE product_id IS NOT NULL AND DATE(created_at) BETWEEN ? AND ?
                    GROUP BY product_id
                    
                    UNION ALL
                    
                    SELECT si.product_id, SUM(s.total_price / 
                        (SELECT COUNT(*) FROM sale_items si2 WHERE si2.sale_id = s.id)
                    ) as revenue, SUM(si.quantity) as units_sold
                    FROM sale_items si
                    JOIN sales s ON si.sale_id = s.id
                    WHERE DATE(s.created_at) BETWEEN ? AND ?
        ...
        case 'slow_moving':
            ...
            $stmt = $db->prepare($query);
            $stmt->execute([$date_to, $date_from, $date_to, $date_from, $date_to]);
            $report_data = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        case 'tax_report':
            ...
            $query = "SELECT 
                        ...
                        SUM(COALESCE(total_price, 0) * 0.12) as vat_amount 
                      FROM sales 
                      WHERE DATE(created_at) BETWEEN ? AND ? 
        ...
        case 'turnover_analysis':
            ...
            FROM products p
            LEFT JOIN (
                SELECT product_id, SUM(quantity) as total_sold, SUM(COALESCE(total_price, 0)) as revenue
                FROM sales 
                WHERE product_id IS NOT NULL AND DATE(created_at) BETWEEN ? AND ?
                GROUP BY product_id
                ...
                SELECT si.product_id, SUM(si.quantity) as total_sold, 
                       SUM(s.total_price / (SELECT COUNT(*) FROM sale_items si2 WHERE si2.sale_id = s.id)) as revenue
                FROM sale_items si
                JOIN sales s ON si.sale_id = s.id
                WHERE DATE(s.created_at) BETWEEN ? AND ?
                GROUP BY si.product_id
            ...
            $stmt = $db->prepare($query);
            $stmt->execute([$date_to, $date_from, $date_from, $date_to, $date_from, $date_to]);
            $report_data = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        case 'profit_analysis':
            ...
            $query = "SELECT 
                ...
                (p.price * 0.7) as estimated_cost,
                (p.price * 0.3) as estimated_profit_per_unit,
                COALESCE(SUM(sales_data.units_sold), 0) as units_sold,
                COALESCE(SUM(sales_data.revenue), 0) as revenue,
                COALESCE(SUM(sales_data.units_sold * p.price * 0.3), 0) as estimated_profit,
                CASE 
                    WHEN SUM(sales_data.revenue) > 0 THEN (SUM(sales_data.units_sold * p.price * 0.3) / SUM(sales_data.revenue)) * 100
                    ELSE 0 
                END as profit_margin_percent
            FROM products p
            LEFT JOIN (
                SELECT product_id, SUM(quantity) as units_sold, SUM(COALESCE(total_price, 0)) as revenue
                FROM sales 
                WHERE product_id IS NOT NULL AND DATE(created_at) BETWEEN ? AND ?
                GROUP BY product_id
                    ...
                    <form method="GET" class="row g-3">
                        <div class="col-md-3">
                            ...
                            <select name="type" class="form-select" onchange="this.form.submit()">
                                ...
                                <option value="tax_report" <?php echo $report_type === 'tax_report' ? 'selected' : ''; ?>>Tax Report</option>
                                    ...
                                    <?php foreach ($report_data as $row): ?>
                                        <tr>
                                            <?php if ($report_type === 'sales'): ?>
                                                ...
                                                <td><?php echo ucfirst(str_replace('_', ' ', $row['payment_method'] ?? 'cash')); ?></td>
                                            <?php elseif ($report_type === 'stock'): ?>
                                                <td><?php echo htmlspecialchars($row['product_name']); ?></td>
                                            ...
                                            <?php elseif ($report_type === 'profit_loss'): ?>
                                                ...
                                                <td>₱<?php echo number_format($row['cogs'], 2); ?></td>
                                                <td>₱<?php echo number_format($row['revenue'] - $row['cogs'], 2); ?></td>
                                                <td><?php echo $row['revenue'] > 0 ? number_format((($row['revenue'] - $row['cogs']) / $row['revenue']) * 100, 1) : 0; ?>%</td>
                                            <?php elseif ($report_type === 'inventory_valuation'): ?>
                                                <td><?php echo htmlspecialchars($row['product_name']); ?></td>
                                                <td><?php echo htmlspecialchars($row['category']); ?></td>
                                            ...
                                            <?php elseif ($report_type === 'supplier_performance'): ?>
                                                <td><?php echo htmlspecialchars($row['supplier']); ?></td>
                                                <td><?php echo number_format($row['products_supplied']); ?></td>
                                            ...
                                            <?php elseif ($report_type === 'slow_moving'): ?>
                                                ...
                                                <td>₱<?php echo number_format($row['inventory_value'], 2); ?></td>
                                                <td><?php echo number_format($row['units_sold']); ?></td>
                                                <td><?php echo number_format($row['days_in_inventory']); ?> days</td>
                                            <?php elseif ($report_type === 'tax_report'): ?>
                                                ...
                                                <td>₱<?php echo number_format($row['daily_sales'] - $row['vat_amount'], 2); ?></td>
                                            <?php elseif ($report_type === 'turnover_analysis'): ?>
                                                <td><?php echo htmlspecialchars($row['product_name']); ?></td>
                                                <td><?php echo number_format($row['stock_quantity']); ?></td>
                                                <td><?php echo number_format($row['total_sold']); ?></td>
                                                <td>₱<?php echo number_format($row['revenue'], 2); ?></td>
                                                <td><?php echo number_format($row['turnover_ratio'], 2); ?></td>
                                                <td>
                                            ...
                                            <?php elseif ($report_type === 'profit_analysis'): ?>
                                                ...
                                                <td>₱<?php echo number_format($row['estimated_profit_per_unit'], 2); ?></td>
                                                <td><?php echo number_format($row['units_sold']); ?></td>
                                                <td>₱<?php echo number_format($row['estimated_profit'], 2); ?></td>
                                                <td><?php echo number_format($row['profit_margin_percent'], 1); ?>%</td>
                                            <?php endif; ?>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
